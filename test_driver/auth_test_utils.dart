...
String generateRandomEmail({
  ...
  final uuid = createCryptoRandomString();
  final testEmail = prefix + uuid + suffix;
  return testEmail;
}
...
Future<String?> emulatorPhoneVerificationCode(String phoneNumber) async {
  ...
  final verificationCodes = List<Map<String, dynamic>>.from(
    responseBody['verificationCodes'] as List);

  return verificationCodes.reversed
    .firstWhere(
      (verificationCode) => verificationCode['phoneNumber'] == phoneNumber,
      orElse: () => <String, dynamic>{ 'code': 'NOT_FOUND' },
    )['code']?.toString();
}
...
Future<EmulatorOobCode?> emulatorOutOfBandCode(
  ...
  final responseBody = Map<String, dynamic>.from(
    jsonDecode(response.body) as Map);
  final oobCodes = List<Map<String, dynamic>>.from(
    responseBody['oobCodes'] as List);
  ...
  return EmulatorOobCode(
    type: type,
    email: oobCode['email']?.toString(),
    oobCode: oobCode['oobCode']?.toString(),
    oobLink: oobCode['oobLink']?.toString(),
  );
}
...
Future<void> ensureSignedIn(String testEmail) async {
  if ( FirebaseAuth.instance.currentUser == null ) {
    try {
      ...
      debugPrint('ensureSignedIn Error $e');
    }
  }
}
